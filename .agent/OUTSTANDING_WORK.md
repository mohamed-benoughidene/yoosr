...
|---|---|---|---|---|---|---|
| [ ] | S-1 | 🔴 Critical | `convex/botFlows.ts` | `save` | Auth check is commented out — any unauthenticated caller can overwrite any bot flow | Uncomment or re-add `if (!identity) throw new Error("Unauthenticated")` at the top of the function |
| [ ] | S-2 | 🔴 Critical | `convex/bot.ts` | `executeNextBlock` (code_action handler) | Uses `new Function(...)` to evaluate user-provided expressions — arbitrary code execution risk | Replace with `expr-eval` library. Import `Parser` from `expr-eval`, call `parser.evaluate(expression, variables)` instead of `new Function` |
| [ ] | S-3 | 🟠 High | `convex/knowledgeBases.ts` | `get`, `create`, `addSource`, `removeSource` | No org check — any authenticated user can read or modify any KB by ID | After resolving `projectId`, verify it belongs to `identity.org_id` via `projects` table lookup |
| [ ] | S-4 | 🟠 High | `convex/bots.ts` | `get` | No org check — any authenticated user can read any bot by ID | Same pattern as S-3: resolve projectId → verify orgId matches identity |
| [ ] | S-5 | 🟠 High | `convex/contacts.ts` | `update`, `findByConversation` | No org check — any authenticated user can update or read any contact | Resolve projectId from the record and verify against `identity.org_id` |
| [ ] | S-6 | 🟠 High | `convex/tags.ts` | `assignTagToConversation`, `removeTagFromConversation` | No org check — any authenticated user can tag any conversation | Resolve projectId from the conversation record and verify against `identity.org_id` |
| [ ] | S-7 | 🟠 High | `convex/analytics.ts` | `getConversationStats`, `getVisitorStats`, `getMessageStats`, `getConversationVolume`, `getTokenUsage`, `getCSATSummary`, `getTagsSummary`, `getSLABreachRate`, `getProjectUsage` | Identity is checked for existence but the requested `projectId` is never verified to belong to the caller's org | Add: fetch the project by `projectId`, assert `project.orgId === identity.org_id`, throw if mismatch |
| [ ] | S-8 | 🟡 Medium | `convex/profiles.ts` | `getByUserId` | No auth or org check — any caller can query any agent profile by userId | Add `await ctx.auth.getUserIdentity()` check; optionally restrict cross-org reads |
| [ ] | S-9 | 🟡 Medium | `convex/http.ts` | `createFromWidget`, `sendFromWidget` (HTTP routes) | No rate limiting — widget endpoints are open to abuse and spam | Implement token-bucket or sliding-window rate limiting at the HTTP handler level using visitor IP or `visitorId` as key |
